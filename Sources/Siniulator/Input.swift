...
@MainActor final class SimulatorInput {
    ...
    init(core: SICoreSimulator, device: Any) throws {
        self.core = core
        self.device = device
    ...
    func activate() async throws {
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            ...
            xpc_connection_send_message_with_reply(connection, message, .global(qos: .userInitiated)) { event in
                if xpc_get_type(event) == XPC_TYPE_ERROR {
                    reply.finish(.failure(SimulatorError(message: "Could not activate simulator input. Retry the connection.")))
                } else { reply.finish(.success(())) }
            }
            DispatchQueue.global().asyncAfter(deadline: .now() + 5) {
                reply.finish(.failure(SimulatorError(message: "Simulator input activation timed out.")))
            }
        }
